...
void main() {
    ...
    bezierSt = translateCoord(bezierSt, vec2(0.2, 0.0));
    //bezierSt = scaleCoord(bezierSt, vec2(0.9, 0.9));
    bezier_ = sdBezier(bezierSt, vec2(.0, .0), vec2(.0, .0), vec2(.0, .0));
    
    bezierSt = translateCoord(bezierSt, vec2(0.4, 0.0));
    //bezierSt = scaleCoord(bezierSt, vec2(0.9, 0.9));
    bezier_ = merge(bezier_, sdBezier(bezierSt, vec2(.0, .0), vec2(.0, .0), vec2(.0, .0)));

    bezierSt = translateCoord(bezierSt, vec2(-1., 0.4));
    bezierSt = rotateCoord(bezierSt, 90.);
    bezier_ = merge(bezier_, sdBezier(bezierSt, vec2(.1, .1), vec2(.1, .9), vec2(1., 1.)));

    stroke_ = stroke(bezier_, .002, 0.2, 0.2);
    resultColor = mix(colorA, resultColor, stroke_);
