...
void main(){
    ...
    float circel_a = distanceCircleBorderToCircleCenter(coord + vec2((((cos(u_time * .5)) * .6)), (((sin(u_time * .6)) * .5))), 0.6, 0.0);
    vec4 result_a = mix(color_a, color_b, circel_a);
    result = result_a;

    float circel_b = distanceCircleBorderToCircleCenter(coord + vec2((((sin(u_time * .5)) * .6)), (((cos(u_time * .6)) * .5))), 0.6, 0.0);
    vec4 result_b = mix(color_a, color_b, circel_b);
    result = mix(result, result_b, 0.5);
    
    float circel_c = distanceCircleBorderToCircleCenter(coord + vec2(-1., -1.), 0.1, 0.0);
