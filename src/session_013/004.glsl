...
vec4 fracture(vec2 x){
    return texture2D(u_buffer1, x/u_resolution);
}
...
void main() {
    ...
    //c = (coord*0.75).xxyy * (sin(u_time*0.5));
    //c = (coord * cos(u_time * 0.2)* 2.75).xxyy;
    ...
    c += fracture(gl_FragCoord.xx + c.xy);
    //c += fracture(gl_FragCoord.yy + c.xy);
    ...
    // += fracture(gl_FragCoord.xy-c.xy) * cos(u_time * 0.3)*2.7 -3.;
   ...
   float bezier = sdBezier(
    ...
    // bezier *= bezier;
    // bezier = clamp(bezier, 0., 1.);
    
    // float cir = circle(coord + off, shape) + 1.5 * 0.5;
    // vec4 circle = vec4(vec3(cir), 1.0);

    vec4 shape = vec4(vec3(1.- stroke(bezier, .003, 0.2, 0.2)), 1.);

    gl_FragColor = adjustedCos(u_time, 1.0, 0.80, 1.0) * (c + shape);
